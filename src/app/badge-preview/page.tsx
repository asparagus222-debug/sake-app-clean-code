...
import {
  ...
  JanomeCupIcon, SakeBottleIcon, YONGO_VARIANTS, YONGO_CLASSIC_COLORS,
...
export default function BadgePreviewPage() {
  return (
    <main className="min-h-screen bg-[#0e1117] text-white p-8 space-y-14">
      ...
      <section className="space-y-4">
        ...
        <div className="flex flex-wrap gap-8 items-end">
          {[
            ...
            { label: '$1000 四合瓶（預設傳統褐）', node: <SakeBottleIcon size={36} colors={YONGO_CLASSIC_COLORS} /> },
      ...
      {/* ── Yongo (四合瓶) variants ── */}
      <section className="space-y-4">
        <h2 className="text-sm font-bold uppercase tracking-widest text-white/50">$1000 四合瓶 — 6 配色方案</h2>
        <div className="grid grid-cols-3 md:grid-cols-6 gap-6">
          {YONGO_VARIANTS.map((v) => (
            <div key={v.id} className="flex flex-col items-center gap-3 p-4 rounded-2xl border border-white/10 bg-white/5">
              <SakeBottleIcon size={52} colors={v.colors} />
      ...
      {/* 四合瓶 at badge size */}
      <section className="space-y-4">
        <h2 className="text-sm font-bold uppercase tracking-widest text-white/50">四合瓶 at badge size (14px) — 行內顯示模擬</h2>
        <div className="flex flex-wrap gap-4 items-center">
          {YONGO_VARIANTS.map((v) => (
            <div key={v.id} className="flex items-center gap-1.5 bg-white/5 rounded-xl px-3 py-2 border border-white/10">
              <SakeBottleIcon size={14} colors={v.colors} />
